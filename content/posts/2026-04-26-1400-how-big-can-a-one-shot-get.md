...
Pretty much everything we build to make agents work better has a shelf life of a few months. Whatever harness or process we ship fills a gap until the next model release makes it unnecessary. Spec-driven systems like [BMAD](https://github.com/bmad-code-org/BMAD-METHOD) and [github-speckit](https://github.com/github/spec-kit) feel like they fit the same pattern. Useful right now, probably obsolete soon. The real question is what a spec should even look like once the underlying model can do more on its own.
...
The application is an AD&D 2e play aid, similar in spirit to D&D Beyond. Character creation, equipment, spells, the buy and sell loop, restrictions like clerics being limited to blunt weapons. It's heading toward a combat aid where weapon speed factors initiative and weapon type interacts with armor type. I picked AD&D 2e on purpose. It has enough rules, modifiers, and bookkeeping to give the spec real surface area, which is exactly what a one-shot needs to be tested against.
